...
const reduced= numbers.reduce((sum, number) => sum + number, 0);
document.getElementById('sumOfArray').textContent=reduced;
...
let multiplyreduced=multiplied.reduce((sum, number)=>sum+number,0);
document.querySelector("#sumOfMultiplied").textContent=multiplyreduced;